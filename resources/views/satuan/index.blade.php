...
@section('toolbars')
    @include('layouts.toolbars', [
        ...
        'par' => '<i class="fa fa-list"></i> Master data satuan',
        'label' => 'Kembali',
        'ajax_button' => '',
    ])
@endsection
...
<script>
    ...
    function del() {
        ...
        if (c.length == 0) {
            ...
            $.post("{{ route('master.satuan.destroy', ':id') }}", {
                '_method': 'DELETE',
                'id': c
            }, function(data) {
    ...
    jQuery(document).ready(function() {
        ...
        $('#tambah').on('click', function() {
            ...
            addUrl = '{{ route('master.satuan.create') }}';
            $('#form_content').html(
        ...
        $('#datatable').on('click', '#edit', function(e) {
            ...
            var id = $(this).data('id');
            addUrl =
                '{{ route('master.satuan.edit', ':id') }}'.replace(':id', id);
            $('#form_content').html(
                '<center><h3><img src="{{ asset('assets/img/loading.gif') }}" class="img-responsive" style="width:350px;height:250px"></h3></center>'
            ).load(addUrl);
        });
