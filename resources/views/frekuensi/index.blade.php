...
@section('title', 'Master data Frekuensi')
...
@section('toolbars')
    @include('layouts.toolbars', [
        'url' => '#',
        'par' => '<i class="fa fa-list"></i> Master data Frekuensi',
        'label' => 'Kembali',
        'ajax_button' => '',
    ])
@endsection
...
<script>
    ...
    function del() {
        ...
        if (c.length == 0) {
            ...
            $.post("{{ route('master.frekuensi.destroy', ':id') }}", {
                '_method': 'DELETE',
                'id': c
            }, function(data) {
    ...
    jQuery(document).ready(function() {
        ...
        $('#tambah').on('click', function() {
            ...
            addUrl = '{{ route('master.frekuensi.create') }}';
        ...
        $('#datatable').on('click', '#edit', function(e) {
            ...
            var id = $(this).data('id');
            addUrl =
                '{{ route('master.frekuensi.edit', ':id') }}'.replace(':id', id);
            $('#form_content').html('<center><h3><img src="{{ asset('assets/img/loading.gif') }}" class="img-responsive" style="width:350px;height:250px"></h3></center>').load(addUrl);
        });
